...
_run_tests() {
  ...
  dotnet test  "./tests/app/UnitTests/GitCommands.Tests/GitCommands.Tests.csproj" \
    --logger "junit;LogFilePath=$ARTIFACTS_DIR/tests.log" --no-build \
...
COMPILE_STATUS="pass"
dotnet build "./tests/app/UnitTests/GitCommands.Tests/GitCommands.Tests.csproj" > /tmp/compile_stdout.log 2> /tmp/compile_stderr.log || {
  COMPILE_STATUS="fail"
}
...
if [ "$COMPILE_STATUS" = "pass" ] && [ "$PATCH_STATUS" = "pass" ]; then
  dotnet build "./tests/app/UnitTests/GitCommands.Tests/GitCommands.Tests.csproj" > /tmp/rebuild_stdout.log 2> /tmp/rebuild_stderr.log || {
    REBUILD_STATUS="fail"
    COMPILE_STATUS="fail"
  }
